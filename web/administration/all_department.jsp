<%-- 
    Document   : all_department
    Created on : Jul 2, 2011, 12:31:04 AM
...
<%!
        ResultSet get_all_department()
	{
            ...
            try
            {
                ...
                query = "select * from Administration.Department";
        ...
        int count_department()
        {
            ...
            try
            {
                ...
                query = "select * from Administration.CountDepartment";
                execute_query = server.server_connection.createStatement();
                result_set = execute_query.executeQuery(query);
                result_set.next();
                count = result_set.getInt("NoOfDepartment");
            }
            catch(SQLException e)
            {
            }
...
<%
    ...
    int count = count_department();
    String page_value = request.getParameter("page");
    int page_number = 1;
    if(count == 0)
    {
        request.setAttribute("warning", "No Department currently exists in the database of college.");
        RequestDispatcher index = request.getRequestDispatcher("/index.jsp");
        index.forward(request, response);
    }

    ResultSet all_department = get_all_department();
                    ...
                    <div class="div-display">
                        <table width="425px">
                            <tr>
                                ...
                                <th width="40%">Department Type</th>
                            </tr>
                            <%
                                if(all_department != null)
                                {
                                    ...
                                    count_records = 0;
                                    while(all_department.next() && count_records < 10)
                                    {
                            %>
                            <tr>
                                <td><%= all_department.getString("Name") %></td>
                                <td><%= all_department.getString("Type") %></td>
                            </tr>
                            <%
                                        count_records++;
                                    }
