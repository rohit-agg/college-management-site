<%-- 
    Document   : all_item
    Created on : Jul 7, 2011, 8:19:27 PM
        ...
        ResultSet get_all_item()
	{
        ...
        int count_item()
        {
            ...
            try
            {
                ...
                query = "select * from Inventory.CountItem";
                execute_query = server.server_connection.createStatement();
                result_set = execute_query.executeQuery(query);
                result_set.next();
                count = result_set.getInt("NoOfItem");
            }
            catch(SQLException e)
            {
            }
...
<%
    ...
    if(count == 0)
    {
        request.setAttribute("warning", "No Item currently exists in the database of college.");
        RequestDispatcher index = request.getRequestDispatcher("/index.jsp");
        index.forward(request, response);
    }

    ResultSet all_item = get_all_item();
        ...
        <div class="page-container">
            ...
            <div class="main">
                ...
                <div class="main-content">
                        ...
                        <table width="500px">
                            ...
                            <%
                                if(all_item != null)
                                {
                                    ...
                                    count_records = 0;
                                    while(all_item.next() && count_records < 10)
                                    {
                            %>
                            <tr>
                                <td><%= all_item.getString("ItemName") %></td>
                                <td><%= all_item.getString("ItemDetails") %></td>
                            </tr>
                            <%
                                        count_records++;
                                    }
                    ...
                    <table class="page">
                        <tr>
                            <td><a href="all_item.jsp?page=<%= page_number - 1 %>">Previous</a></td>
                            <%
                                for(int i = (page_number - 5 > 1 ? page_number - 5 : 1); i <= (page_number - 1); i++)
                                {
                            %>
                            <td><a href="all_item.jsp?page=<%= i %>"><%= i %></a></td>
                            <%
                                }
                            %>
                            <td><a href="all_item.jsp?page=<%= page_number %>"><%= page_number %></a></td>
                            <%
                            ...
                            %>
                            <td><a href="all_item.jsp?page=<%= page_number + 1 %>">Next</a></td>
                        </tr>
                    </table>
                </div>
            </div>
